...
export default function DeedSheet() {
      ...
      if (mapUrl) {
        try {
          ...
          if (mapRes.ok) {
            ...
            ms.addText("★ Subject property · numbered pins are comparable sales in ledger order", {
  ...
  const buildMap = async () => {
    ...
    try {
      ...
      if (!subjCoord) { setMapUrl(null); setMapSig(sig); return; }
      ...
      // Compute a bounding box around all pins with padding
      ...
      const markers = [
        `lonlat:${subjCoord[0]},${subjCoord[1]};type:awesome;color:%238E3B2F;size:x-large;icon:star`,
        ...compCoords
          .map((coord, i) =>
            coord
              ? `lonlat:${coord[0]},${coord[1]};type:material;color:%231F3D2B;size:large;text:${i + 1}`
              : null
          )
          .filter(Boolean),
      ].join("|");

      const url = `https://maps.geoapify.com/v1/staticmap?style=positron&width=900&height=500&area=${encodeURIComponent(rect)}&marker=${markers}&scaleFactor=2&apiKey=${GEOAPIFY_KEY}`;
      setMapUrl(url);
      setMapSig(sig);
    } catch (err) {
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <button
              ...
              disabled={slidesBusy}
              style={{ padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: slidesBusy ? "wait" : "pointer", borderRadius: 3, border: "1px solid #2E5540", background: "transparent", color: "#8FAE9B" }}
            >
              {slidesBusy ? "Building…" : "Full CMA (Slides)"}
            </button>
            <button
              onClick={signOut}
                ...
                {mapUrl ? (
                  <>
                    ...
                    <div style={{ fontSize: 10.5, color: "#8A7E63", marginTop: 6, fontStyle: "italic" }}>
                      ★ Subject property · numbered pins are comparable sales in ledger order
                    </div>
                  </>
                ) : (
