...
export default function DeedSheet() {
  ...
  // ---------- subscription ----------
  const [sub, setSub] = useState({ loading: true, status: null, hasBilling: false });
  ...
  useEffect(() => {
    ...
    else setSub({ loading: true, status: null, hasBilling: false });
  ...
  const subActive = ["active", "trialing"].includes(sub.status);
  const billingConfigured = sub.status !== null || sub.hasBilling;

  if (!sub.loading && !subActive && billingConfigured) {
    return (
      <div style={{ background: "#1F3D2B", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Libre Franklin', Arial, sans-serif", padding: 20 }}>
        <style>{fonts}</style>
