    ...
    try {
      ...
      adjusted.forEach((c) => {
        ...
        cs.addText(c.address, {
          x: 0.8, y: 1.15, w: c.photo ? 8.7 : 11.7, h: 0.7, fontFace: SERIF, fontSize: 30, color: INK, bold: true,
        });
        cs.addText(
          `${c.beds} bd / ${c.baths} ba · ${c.sqft.toLocaleString()} sqft · sold ${c.sold}`,
          { x: 0.8, y: 1.85, w: c.photo ? 8.7 : 11.7, h: 0.4, fontFace: SANS, fontSize: 14, color: MUTE }
        );
        if (c.photo) {
          cs.addImage({
            data: c.photo,
            x: 9.7, y: 0.7, w: 2.9, h: 2.1,
            sizing: { type: "cover", w: 2.9, h: 2.1 },
          });
        }
        const rows = [
          [
