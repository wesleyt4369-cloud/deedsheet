...
export default function DeedSheet() {
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      setProfileMsg("Saved. New reports will start with this branding.");
    } catch (e) {
      setProfileMsg("Could not save: " + (e.message || "unknown error"));
    } finally {
  ...
  const applyProfile = () => {
    ...
    setProfileMsg("Branding restored from your profile.");
  ...
  const fonts = `
    ...
    .ds-rail { width: 196px; flex-shrink: 0; background: #12291B; border-right: 1px solid #2E5540;
      padding: 18px 12px; display: flex; flex-direction: column; gap: 2px; }
    .ds-main { flex: 1; min-width: 0; }
    .ds-nav { display: block; width: 100%; text-align: left; padding: 9px 11px; border-radius: 4px; border: none;
      background: transparent; color: #8FAE9B; font-size: 13px; font-family: inherit; cursor: pointer; transition: all .13s ease; }
    .ds-nav:hover { background: rgba(168,133,60,.12); color: #EFE7D3; }
    .ds-nav-on { background: rgba(168,133,60,.18); color: #EFE7D3; font-weight: 600; }
    ...
    .ds-stat { background: rgba(255,255,255,.04); border: 1px solid #2E5540; border-radius: 4px; padding: 13px 15px; }
    @media (max-width: 900px) {
      .ds-shell { flex-direction: column; }
      .ds-rail { width: auto; flex-direction: row; overflow-x: auto; border-right: none;
        border-bottom: 1px solid #2E5540; padding: 10px; gap: 6px; }
  ...
  const railBtn = (id, label, onClick) => (
    <button
      key={id}
  ...
  return (
    <div style={{ background: "#1F3D2B", minHeight: "100vh", fontFamily: "'Libre Franklin', sans-serif" }}>
      ...
      <div className="ds-rail chrome">
        <div className="ds-railhead" style={{ padding: "0 11px 14px" }}>
          <div className="fraunces" style={{ color: "#EFE7D3", fontSize: 19, fontWeight: 700 }}>DeedSheet</div>
          <div style={{ color: "#6F8C7B", fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", marginTop: 2 }}>
            CMA reports in two minutes
          </div>
        </div>
        {railBtn("home", "Home")}
        <button className="ds-nav" onClick={newReport} style={{ color: "#D9C48F", fontWeight: 600 }}>+ New report</button>
        {railBtn("inputs", currentDraftId || inEditor ? "Current report" : "Report editor")}
        {railBtn("drafts", "My drafts")}
        {railBtn("shares", "Shared links")}
        ...
        {sub.billingEnabled && !sub.owner && (
          <button className="ds-nav ds-nav-sm" onClick={openBillingPortal} disabled={billingBusy}>Billing</button>
        )}
        <button className="ds-nav ds-nav-sm" onClick={() => { window.location.href = "/reset"; }}>Password</button>
        <button className="ds-nav ds-nav-sm" onClick={signOut} title={session.user.email}>Sign out</button>
      </div>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <div style={{ color: "#8FAE9B", fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase" }}>
              {inEditor ? "Report editor" : "DeedSheet"}
            </div>
          </div>
      ...
      {view === "home" && (() => {
        const now = new Date();
        const thisMonth = drafts.filter((d) => {
        ...
        const shareFor = (name) =>
          shares.find((x) => x.address && name && name.toLowerCase().includes(x.address.toLowerCase()));
        const hour = now.getHours();
        const greeting = hour < 12 ? "Good morning" : hour < 18 ? "Good afternoon" : "Good evening";
        const firstName = (agent.name || "").split(" ")[0];

        return (
          <div className="chrome" style={{ maxWidth: 960, margin: "0 auto", padding: "26px 20px 60px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 14, marginBottom: 22 }}>
              <div>
                <div className="fraunces" style={{ fontSize: 24, fontWeight: 700, color: "#EFE7D3" }}>
                  {greeting}{firstName && firstName !== "Your" ? `, ${firstName}` : ""}
                </div>
                <div style={{ fontSize: 12.5, color: "#8FAE9B", marginTop: 3 }}>
                  {now.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}
                </div>
              </div>
              <button
                onClick={newReport}
                style={{ padding: "12px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer", borderRadius: 4, border: "none", background: "#8E3B2F", color: "#EFE7D3" }}
              >
                + New CMA
              </button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10, marginBottom: 24 }}>
              {[
                [thisMonth, "Reports this month"],
                [totalViews, "Client views"],
                [awaiting, "Awaiting client"],
                [drafts.length, "Saved reports"],
              ].map(([n, label]) => (
                <div key={label} className="ds-stat">
                  <div className="fraunces" style={{ fontSize: 26, fontWeight: 700, color: "#A8853C" }}>{n}</div>
                  <div style={{ fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: "#8FAE9B", marginTop: 3 }}>{label}</div>
                </div>
              ))}
            </div>

            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
              <div style={{ fontSize: 10.5, letterSpacing: "0.16em", textTransform: "uppercase", color: "#8FAE9B", fontWeight: 700 }}>
                Recent reports
              </div>
              {drafts.length > 6 && (
                <button onClick={() => setView("drafts")} style={{ background: "none", border: "none", color: "#D9C48F", fontSize: 12, cursor: "pointer", textDecoration: "underline" }}>
                  See all {drafts.length}
                </button>
              )}
            </div>

            {drafts.length === 0 ? (
              <div style={{ background: "#EFE7D3", border: "1px solid #C9BC9C", borderRadius: 4, padding: "30px 24px", textAlign: "center" }}>
                <div className="fraunces" style={{ fontSize: 18, fontWeight: 600, color: "#26221A" }}>No reports yet</div>
                <div style={{ fontSize: 13, color: "#6B6252", marginTop: 6, lineHeight: 1.6 }}>
                  {savedProfile
                    ? "Start your first CMA — your branding is already set."
                    : "Tip: open a new report and save your name, brokerage and logo as your profile. Every report after that starts pre-branded."}
                </div>
                <button onClick={newReport} style={{ marginTop: 16, padding: "10px 22px", fontSize: 13.5, fontWeight: 700, cursor: "pointer", borderRadius: 3, border: "none", background: "#1F3D2B", color: "#EFE7D3" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
                {drafts.slice(0, 6).map((d) => {
                  const sh = shareFor(d.name);
                  return (
                    <button key={d.id} className="ds-card" onClick={() => openDraft(d.id)}>
                      <div className="fraunces" style={{ fontSize: 15.5, fontWeight: 600 }}>{d.name}</div>
                      <div style={{ fontSize: 11.5, color: "#6B6252", marginTop: 3 }}>
                        Edited {new Date(d.updated_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                      </div>
                      <div style={{ marginTop: 8 }}>
                        {sh ? (
                          sh.view_count > 0 ? (
                            <span style={{ fontSize: 9.5, letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700, padding: "2px 7px", borderRadius: 2, background: "#1F3D2B", color: "#EFE7D3" }}>
                              Opened {sh.view_count}×
                            </span>
                          ) : (
                            <span style={{ fontSize: 9.5, letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700, padding: "2px 7px", borderRadius: 2, background: "#D8CCAC", color: "#6B6252" }}>
                              Shared · not opened
                            </span>
                          )
                        ) : (
                          <span style={{ fontSize: 9.5, letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700, padding: "2px 7px", borderRadius: 2, background: "#E7DDC2", color: "#8A7E63" }}>
                            Draft
                          </span>
                        )}
                      </div>
      ...
      {view === "shares" && (
        <div className="chrome" style={{ maxWidth: 760, margin: "28px auto", padding: "0 20px 60px" }}>
          ...
          {sharesBusy && shares.length === 0 && (
            <div style={{ ...cardStyle, textAlign: "center", color: "#6B6252", fontSize: 14 }}>Loading…</div>
          )}
          {!sharesBusy && shares.length === 0 && (
              ...
              <div key={sh.id} style={{ ...cardStyle, borderLeft: `3px solid ${viewed ? "#1F3D2B" : "#C9BC9C"}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                    ...
                    <button onClick={() => deleteShare(sh.id)} title="Revoke this link"
          ...
          <div style={cardStyle}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10, marginBottom: 12 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: "#6B6252", marginTop: 2 }}>
                  Set this up once — every new report starts with your branding.
                </div>
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {savedProfile && (
                  <button
                    ...
                    style={{ padding: "8px 14px", fontSize: 12.5, fontWeight: 600, cursor: "pointer", borderRadius: 3, border: "1px solid #A99C7A", background: "transparent", color: "#6B6252" }}
                  >
                    Reset to my profile
                  </button>
                )}
                <button
                  ...
                  style={{ padding: "8px 16px", fontSize: 12.5, fontWeight: 700, cursor: profileBusy ? "wait" : "pointer", borderRadius: 3, border: "none", background: "#1F3D2B", color: "#EFE7D3" }}
                >
                  {profileBusy ? "Saving…" : savedProfile ? "Update my profile" : "Save as my profile"}
                </button>
              </div>
            </div>
            {profileMsg && (
              <div style={{ fontSize: 12.5, color: "#1F3D2B", fontWeight: 600, marginBottom: 10 }}>{profileMsg}</div>
            )}
