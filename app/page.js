  ...
  const exportSlides = async () => {
    ...
    try {
      ...
      s.addText("PREPARED EXCLUSIVELY FOR THE PROPERTY OWNER", {
        x: 0.6, y: 1.3, w: 12.1, h: 0.4, align: "center",
        fontFace: SANS, fontSize: 11, color: RED, charSpacing: 6, bold: true,
      });
      s.addText("Comparative Market Analysis", {
        x: 0.6, y: 1.9, w: 12.1, h: 1.1, align: "center",
        fontFace: SERIF, fontSize: 44, color: GREEN, bold: true,
      });
      s.addText(`${subject.address} · ${subject.city}`, {
        x: 0.6, y: 3.1, w: 12.1, h: 0.6, align: "center",
        fontFace: SERIF, fontSize: 22, color: INK,
      });
      s.addText(
        `${subject.beds} bed · ${subject.baths} bath · ${subject.sqft.toLocaleString()} sqft · ${subject.lot} lot · built ${subject.year}`,
        { x: 0.6, y: 3.7, w: 12.1, h: 0.4, align: "center", fontFace: SANS, fontSize: 13, color: MUTE }
      );
      ...
      adjusted.forEach((c) => {
        ...
        cs.addTable(rows, {
          x: 0.8, y: 2.6, w: 11.7, colW: [8.2, 3.5],
