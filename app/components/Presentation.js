...
export default function Presentation({
  ...
  useEffect(() => {
    ...
    const mq = window.matchMedia("(max-width: 900px), (max-height: 620px)");
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const mq = window.matchMedia("(orientation: landscape) and (max-height: 620px)");
    const sync = () => {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      let w = Math.round(r.width) || el.clientWidth;
      let h = Math.round(r.height) || el.clientHeight;
      if (!w || !h) {
        // last resort: size from the window so the slide is never invisible
        w = Math.max(200, window.innerWidth - 90);
        h = Math.max(140, window.innerHeight - 150);
      }
  ...
  const renderDetailPanel = () => {
    ...
    const stat = (label, value) => (
      <div key={label} style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: "10px", letterSpacing: "0.18em", textTransform: "uppercase", color: `${PARCH}88` }}>{label}</div>
        <div style={{ fontSize: "16px", fontWeight: 700, color: PARCH, marginTop: 2, overflowWrap: "anywhere" }}>{value || "—"}</div>
      </div>
    );
        ...
        >
          ...
          <div style={{ padding: "16px 18px 14px", borderBottom: `1px solid ${BRASS}55`, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              <div className="ds-head" style={{ fontSize: "21px", fontWeight: 700, color: PARCH, textTransform: "uppercase", letterSpacing: "0.02em", overflowWrap: "anywhere" }}>
                {c.address}
              </div>
              <div style={{ fontSize: "12px", color: `${PARCH}99`, marginTop: 3 }}>
                {isSubject ? subject.city : (subject.city || "")}
              </div>
              <div className="ds-head" style={{ fontSize: "24px", fontWeight: 700, color: BRASS, marginTop: 8 }}>
                {isSubject ? fmt(mid) : fmt(c.price)}
                {isSubject && <span style={{ fontSize: "11px", color: `${PARCH}99`, fontWeight: 400, marginLeft: 8 }}>{L.positionLead}</span>}
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
              <span style={{ background: isSubject ? RED : statusColor, color: "#fff", fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "4px 9px", borderRadius: 3 }}>
          ...
          {pics.length > 0 ? (
            ...
            <div style={{ padding: "26px 18px", textAlign: "center", color: `${PARCH}77`, fontSize: "12px", fontStyle: "italic" }}>
          ...
          {(isSubject ? subject.features : c.remarks) && (
            <div style={{ padding: "14px 18px", borderBottom: `1px solid ${BRASS}33` }}>
              <div style={{ fontSize: "11px", letterSpacing: "0.22em", textTransform: "uppercase", color: BRASS, fontWeight: 700, marginBottom: 10 }}>
                {isSubject ? "Features" : "Agent remarks"}
              </div>
              <div style={{ fontSize: "14px", lineHeight: 1.75, color: `${PARCH}DD`, whiteSpace: "pre-wrap", overflowWrap: "anywhere", wordBreak: "break-word" }}>
                {isSubject ? subject.features : c.remarks}
              </div>
            </div>
          )}
          ...
          {rows.length > 0 && (
            <div style={{ padding: "14px 18px 8px" }}>
              ...
              {rows.map(([k, v]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "7px 0", borderBottom: `1px solid ${PARCH}18`, fontSize: "13px" }}>
  ...
  const renderSlide = (slide) => {
    ...
    switch (slide.type) {
      case "cover":
        return (
          <div style={{ height: "100%", position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "3.6% 8%", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: "6%", right: "5%", width: 100, height: 100, borderRadius: "50%", border: `2px solid ${RED}`, display: "flex", alignItems: "center", justifyContent: "center", transform: "rotate(-10deg)", opacity: 0.75 }}>
              <div style={{ textAlign: "center", color: RED, fontSize: "9px", letterSpacing: "0.12em", textTransform: "uppercase", lineHeight: 1.6, fontWeight: 700, fontFamily: "'Libre Franklin', Arial, sans-serif" }}>
            ...
            {ed("cover.eyebrow", L.coverEyebrow,
              { fontSize: "11px", letterSpacing: "0.34em", textTransform: "uppercase", color: RED, fontWeight: 700, fontFamily: "'Libre Franklin', Arial, sans-serif" },
              { block: true })}

            {ed("cover.title", L.coverTitle,
              { fontSize: "52px", fontWeight: 700, color: GREEN, marginTop: "1.6%", lineHeight: 1.08, letterSpacing: "-0.01em" },
              { block: true, className: "ds-head" })}

            <div className="ds-head" style={{ fontSize: "23px", color: INK, marginTop: "1.5%" }}>
              {subject.address} · {subject.city}
            </div>
            <div style={{ fontSize: "13px", color: MUTE, marginTop: "0.8%" }}>
            ...
            {subject.photo && (
              <img src={subject.photo} alt={subject.address}
                style={{ marginTop: "2.2%", width: "42%", maxHeight: "26%", minHeight: 0, objectFit: "cover", border: `1.5px solid ${BRASS}`, boxShadow: "0 6px 18px rgba(0,0,0,0.14)" }} />
            )}

            <div style={{ width: 52, height: 1.5, background: BRASS, margin: "2.4% 0 1.9%", flexShrink: 0 }} />

            {agent.logo && (
              <img src={agent.logo} alt={agent.brokerage}
                style={{ maxWidth: "23%", maxHeight: "11%", objectFit: "contain", marginBottom: "1.4%" }} />
            )}

            <div style={{ fontSize: "13px", color: MUTE, flexShrink: 0 }}>
              Prepared by <span style={{ color: INK, fontWeight: 600 }}>{agent.name}</span> · {agent.brokerage}
            </div>
            <div style={{ fontSize: "11px", color: RED, marginTop: 4, letterSpacing: "0.1em", textTransform: "uppercase", flexShrink: 0, paddingBottom: 2 }}>
          ...
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5% 7%" }}>
            ...
            <div className="ds-comprow" style={{ flex: 1, display: "flex", gap: "5%", alignItems: "stretch", marginTop: "2.5%", minHeight: 0 }}>
              ...
              <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                <div className="ds-head" style={{ fontSize: "37px", fontWeight: 700, color: GREEN, letterSpacing: "-0.01em" }}>
                  {agent.name}
                </div>
                <div style={{ fontSize: "14px", color: MUTE, marginTop: 6 }}>
                ...
                {ed("intro.body", agent.bio || "Add a short bio in Agent inputs — a couple of lines about your experience in this neighborhood.",
                  { fontSize: "19px", lineHeight: 1.75, color: INK, whiteSpace: "pre-wrap", overflowWrap: "anywhere" },
                  { block: true, className: "ds-head" })}
                <div style={{ fontSize: "16px", color: INK, fontWeight: 600, marginTop: "4%" }}>
                  {agent.phone}
                </div>
              </div>
            </div>
      ...
      case "details": {
        ...
        const featRaw = (subject.features || "").trim();
        const feats = featRaw.split(/\n|,/).map((f) => f.trim()).filter(Boolean);
        // A pasted listing description is prose, not a feature list — treat it differently
        const isProse = featRaw.length > 170 || feats.some((f) => f.length > 46);
        const half = Math.ceil(rows.length / 2);
        const col = (list) => (
          <div style={{ flex: 1, minWidth: 0 }}>
            {list.map(([k, v]) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "1.5% 0", borderBottom: `1px solid ${BRASS}22`, fontSize: "14px" }}>
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "4.5% 7%" }}>
            ...
            <div className="ds-head" style={{ fontSize: "28px", fontWeight: 700, color: INK, marginTop: "1.5%" }}>
              {subject.address}
            </div>
            <div className="ds-onlight" style={{ flex: 1, display: "flex", gap: "5%", marginTop: "2.5%", minHeight: 0, overflow: "auto" }}>
            ...
            {feats.length > 0 && (
              <div style={{ flexShrink: 0, marginTop: "2%", paddingTop: "2%", borderTop: `1.5px solid ${BRASS}` }}>
                ...
                {isProse ? (
                  <div style={{
                    fontSize: 15, lineHeight: 1.65, color: INK, whiteSpace: "pre-wrap",
                    overflowWrap: "anywhere", display: "-webkit-box", WebkitLineClamp: 6,
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5%", rowGap: 8 }}>
                    {feats.slice(0, 10).map((f, i) => (
                      <span key={i} style={{ background: CARD, color: INK, fontSize: 13, padding: "5px 11px", borderRadius: 3, overflowWrap: "anywhere", maxWidth: "100%" }}>{f}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5% 8%" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", minHeight: 0 }}>
              <div className="ds-head" style={{ textAlign: "center", fontSize: "77px", fontWeight: 700, color: RED, lineHeight: 1, letterSpacing: "-0.02em" }}>
                {fmt(mid)}
              </div>
              <div style={{ textAlign: "center", fontSize: "12px", letterSpacing: "0.3em", textTransform: "uppercase", color: MUTE, marginTop: "1.6%", fontFamily: "'Libre Franklin', Arial, sans-serif", fontWeight: 600 }}>
                {L.positionLead}
              </div>
              <div style={{ width: "70%", margin: "6% auto 0" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: "3%" }}>
                  <div>
                    <div className="ds-head" style={{ fontSize: "28px", fontWeight: 600, color: INK }}>{fmt(lo)}</div>
                    <div style={{ fontSize: "10px", letterSpacing: "0.22em", color: MUTE, fontFamily: "'Libre Franklin', Arial, sans-serif", marginTop: 3 }}>LOW</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div className="ds-head" style={{ fontSize: "28px", fontWeight: 600, color: INK }}>{fmt(hi)}</div>
                    <div style={{ fontSize: "10px", letterSpacing: "0.22em", color: MUTE, fontFamily: "'Libre Franklin', Arial, sans-serif", marginTop: 3 }}>HIGH</div>
                  </div>
                </div>
              </div>
            </div>
            <div style={{ textAlign: "center", fontSize: "13px", color: MUTE, fontStyle: "italic" }}>
      ...
      case "map": {
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "2.6% 3.6% 2.2%" }}>
            ...
            <div className="ds-strip" style={{ display: "flex", gap: 6, overflowX: "auto", flexShrink: 0, margin: "1.1% 0 1%", paddingBottom: 4 }}>
              {strip.map((it, i) => (
                ...
                >
                  ...
                  <div style={{ padding: "4px 6px", fontSize: "10px", color: INK, fontWeight: 600, lineHeight: 1.3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {it.addr}
                  </div>
                </button>
              ))}
            ...
            <div className="ds-mapcap" style={{ flexShrink: 0, fontSize: "11px", color: MUTE, fontStyle: "italic", paddingTop: 7 }}>
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "4.5% 7%" }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: "1.5%", marginTop: "2%" }}>
              <div className="ds-head" style={{ fontSize: "26px", fontWeight: 700, color: BRASS }}>{slide.i + 1}</div>
              <div className="ds-head" style={{ fontSize: "35px", fontWeight: 700, color: INK, letterSpacing: "-0.01em" }}>{c.address}</div>
            </div>
            <div style={{ fontSize: "14px", color: MUTE, marginTop: 6, letterSpacing: "0.02em" }}>
            ...
            <div className="ds-comprow" style={{ flex: 1, display: "flex", gap: "3.5%", marginTop: "3%", minHeight: 0 }}>
              <div style={{ flex: c.photo ? 1.45 : 1, minWidth: 0 }}>
                <div style={{ background: CARD, padding: "3.5% 4%" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "18px", fontWeight: 700, color: INK, paddingBottom: 8 }}>
                    <span>Sale price</span><span>{fmt(c.price)}</span>
                  </div>
                  {(c.adjustments || []).map((a, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: "14px", color: MUTE, padding: "5px 0" }}>
                      <span>{a.label}</span>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "20px", fontWeight: 700, color: GREEN, paddingTop: 10, marginTop: 8, borderTop: `1px dashed ${BRASS}` }}>
      ...
      case "strategy":
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5% 8%" }}>
            ...
            <div className="ds-onlight" style={{ flex: 1, display: "flex", alignItems: "center", minHeight: 0, overflow: "auto" }}>
              <div style={{ borderLeft: `4px solid ${BRASS}`, paddingLeft: "3.5%" }}>
                {ed("strategy.body", strategyDefault,
                  { fontSize: "25px", lineHeight: 1.7, color: INK, whiteSpace: "pre-wrap", overflowWrap: "anywhere" },
                  { block: true, className: "ds-head" })}
              </div>
            </div>
          </div>
          ...
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5% 10%" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", minHeight: 0 }}>
              <div style={{ background: CARD, padding: "3.5% 5%" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "19px", fontWeight: 700, color: INK, paddingBottom: 10 }}>
                  <span>Sale price</span><span>{fmt(netBase)}</span>
                </div>
                {netLines.map(([label, v]) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", fontSize: "16px", color: MUTE, padding: "6px 0 6px 4%" }}>
                    <span>{label}</span>
                    <span style={{ color: RED, fontWeight: 600 }}>−{fmt(v)}</span>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", fontSize: "27px", fontWeight: 700, color: GREEN, paddingTop: 14, marginTop: 12, borderTop: `1.5px solid ${BRASS}` }}>
                  <span>Estimated net to seller</span><span>{fmt(netProceeds)}</span>
                </div>
              </div>
              <div style={{ fontSize: "11px", color: "#8A7E63", fontStyle: "italic", marginTop: "2.5%", textAlign: "center" }}>
      ...
      case "custom": {
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5.5% 8%" }}>
            {box("title", c.title, { fontFamily: "'Fraunces', Georgia, serif", fontSize: "40px", fontWeight: 700, color: GREEN, letterSpacing: "-0.01em" })}
            <div style={{ width: 62, height: 1.5, background: BRASS, margin: "2.8% 0" }} />
            <div className="ds-onlight" style={{ flex: 1, minHeight: 0, overflow: "auto" }}>
              {box("body", c.body, { fontFamily: "'Fraunces', Georgia, serif", fontSize: "21px", lineHeight: 1.75, color: INK, whiteSpace: "pre-wrap", overflowWrap: "anywhere" })}
            </div>
          </div>
        );
      }
      ...
      case "close":
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "6% 9%" }}>
            <div style={{ fontSize: "11px", letterSpacing: "0.32em", textTransform: "uppercase", color: RED, fontWeight: 700, fontFamily: "'Libre Franklin', Arial, sans-serif", marginBottom: "3%" }}>
              Thank you
            </div>
            {agent.logo && (
              <img src={agent.logo} alt={agent.brokerage} style={{ maxWidth: "22%", maxHeight: "16%", objectFit: "contain", marginBottom: "3%" }} />
            )}
            <div className="ds-head" style={{ fontSize: "42px", fontWeight: 700, color: GREEN, letterSpacing: "-0.01em" }}>{agent.name}</div>
            <div style={{ width: 46, height: 1.5, background: BRASS, margin: "3% 0" }} />
            <div style={{ fontSize: "17px", color: MUTE }}>{agent.brokerage} · {agent.license}</div>
            <div style={{ fontSize: "17px", color: INK, marginTop: 6, fontWeight: 600 }}>{agent.phone}</div>
            <p style={{ fontSize: "10px", color: "#8A7E63", lineHeight: 1.7, marginTop: "auto", fontStyle: "italic", maxWidth: "78%" }}>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: DARK, zIndex: 9999, display: "flex", flexDirection: "column", fontFamily: "'Libre Franklin', Arial, sans-serif" }}>
      ...
      <div
        ...
        className="ds-stage"
        style={{ flex: 1, position: "relative", display: "flex", alignItems: "center", justifyContent: "center", padding: "16px 56px", minHeight: 0, overflow: "hidden" }}
      >
        <div style={{ width: Math.max(160, Math.round(CANVAS_W * (scale || 0.5))), height: Math.max(90, Math.round(CANVAS_H * (scale || 0.5))), position: "relative", flexShrink: 0 }}>
          <div className="ds-slide ds-slidebox" key={slides[idx]?.key || idx}
            style={{
              width: CANVAS_W, height: CANVAS_H,
              transform: `scale(${scale || 0.5})`, transformOrigin: "top left",
              position: "absolute", top: 0, left: 0,
              background: PARCH, boxShadow: "0 16px 46px rgba(0,0,0,.5)",
