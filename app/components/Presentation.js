  ...
  const renderDetailPanel = () => {
    ...
    const stat = (label, value) => (
      <div key={label} style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: "clamp(11px, 1.3vw, 16px)", fontWeight: 700, color: PARCH, marginTop: 2 }}>{value || "—"}</div>
      </div>
    );
    ...
    return (
      ...
      >
        <div
          ...
          style={{ width: "min(62%, 720px)", height: "100%", background: GREEN, overflowY: "auto", boxShadow: "-10px 0 30px rgba(0,0,0,.35)" }}
        >
          {/* Header */}
          <div style={{ padding: "3% 4% 2.4%", borderBottom: `1px solid ${BRASS}55`, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
            <div style={{ minWidth: 0 }}>
              <div className="ds-head" style={{ fontSize: "clamp(13px, 1.65vw, 21px)", fontWeight: 700, color: PARCH, textTransform: "uppercase", letterSpacing: "0.02em" }}>
                {c.address}
              </div>
          ...
          {pics.length > 0 ? (
            <div style={{ position: "relative", background: "#000" }}>
              <img src={pics[pi]} alt="" style={{ width: "100%", aspectRatio: "16 / 10", objectFit: "cover", display: "block" }} />
              {pics.length > 1 && (
                <>
            ...
            <div style={{ padding: "6% 4%", textAlign: "center", color: `${PARCH}77`, fontSize: "clamp(9px, 1.05vw, 12.5px)", fontStyle: "italic" }}>
              No photos added for this property yet.
            </div>
          )}
          {pics.length > 1 && (
            <div style={{ display: "flex", gap: 5, padding: "8px 4% 0", overflowX: "auto" }}>
              {pics.map((p, i2) => (
          ...
          {/* Stats */}
          <div style={{ display: "flex", gap: "3%", padding: "3.5% 4%", borderBottom: `1px solid ${BRASS}33` }}>
            {stat("Bed", c.beds)}
            {stat("Bath", c.baths)}
          ...
          {(isSubject ? subject.features : c.remarks) && (
            <div style={{ padding: "3.5% 4%", borderBottom: `1px solid ${BRASS}33` }}>
              ...
              <div style={{ fontSize: "clamp(9.5px, 1.1vw, 13.5px)", lineHeight: 1.75, color: `${PARCH}DD`, whiteSpace: "pre-wrap" }}>
                {isSubject ? subject.features : c.remarks}
              </div>
            </div>
          )}
            ...
            <div style={{ padding: "3.5% 4% 5%" }}>
              ...
              {rows.map(([k, v]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "7px 0", borderBottom: `1px solid ${PARCH}18`, fontSize: "clamp(9px, 1.05vw, 13px)" }}>
                  <span style={{ color: `${PARCH}99` }}>{k}</span>
                  <span style={{ color: PARCH, fontWeight: 600, textAlign: "right" }}>{v}</span>
                </div>
              ))}
            </div>
          )}
  ...
  const renderSlide = (slide) => {
    ...
    switch (slide.type) {
              ...
              <div style={{ flexShrink: 0, marginTop: "2%", paddingTop: "2%", borderTop: `1.5px solid ${BRASS}` }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5%", rowGap: 8 }}>
                  {feats.slice(0, 10).map((f, i) => (
                    <span key={i} style={{ background: CARD, color: INK, fontSize: "clamp(9px, 1.1vw, 13px)", padding: "5px 11px", borderRadius: 3 }}>{f}</span>
                  ))}
                </div>
              </div>
            )}
      ...
      case "map": {
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "3.2% 4.5% 2.6%" }}>
            {eyebrow(L.map, "map.eyebrow")}

            {/* Comp filmstrip */}
            <div style={{ display: "flex", gap: 7, overflowX: "auto", flexShrink: 0, margin: "1.6% 0 1.4%", paddingBottom: 4 }}>
              {strip.map((it, i) => (
                <button
                  key={i}
                  onClick={() => focusPin(i)}
                  ...
                  style={{
                    flex: "0 0 auto", width: "clamp(96px, 12%, 168px)", padding: 0, cursor: "pointer", textAlign: "left",
    ...
    <div style={{ position: "fixed", inset: 0, background: DARK, zIndex: 9999, display: "flex", flexDirection: "column", fontFamily: "'Libre Franklin', Arial, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14, padding: "10px 0 14px" }}>
        ...
        <div style={{ color: SAGE, fontSize: 11.5, opacity: .8 }}>
          {idx + 1} / {slides.length} · ← → to navigate{editing ? " · click any text to edit · + adds a slide at that spot" : ""}
        </div>
      </div>
    </div>
  );
