...
export default function Presentation({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      streets.addTo(map);
      L.control.layers({ "Streets": streets, "Overhead (Satellite)": satellite }, null, { collapsed: false, position: "bottomleft" }).addTo(map);
  ...
  const renderDetailPanel = () => {
    ...
    const stat = (label, value) => (
      <div key={label} style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: "clamp(7px, 0.8vw, 9.5px)", letterSpacing: "0.18em", textTransform: "uppercase", color: `${PARCH}88` }}>{label}</div>
        <div style={{ fontSize: "clamp(11px, 1.3vw, 16px)", fontWeight: 700, color: PARCH, marginTop: 2, overflowWrap: "anywhere" }}>{value || "—"}</div>
      </div>
    );
        ...
        >
          ...
          <div style={{ padding: "16px 18px 14px", borderBottom: `1px solid ${BRASS}55`, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              <div className="ds-head" style={{ fontSize: "clamp(13px, 1.65vw, 21px)", fontWeight: 700, color: PARCH, textTransform: "uppercase", letterSpacing: "0.02em", overflowWrap: "anywhere" }}>
                {c.address}
              </div>
              <div style={{ fontSize: "clamp(9px, 1.05vw, 12.5px)", color: `${PARCH}99`, marginTop: 3 }}>
                {isSubject ? subject.city : (subject.city || "")}
              </div>
              <div className="ds-head" style={{ fontSize: "clamp(14px, 1.9vw, 25px)", fontWeight: 700, color: BRASS, marginTop: 8 }}>
                {isSubject ? fmt(mid) : fmt(c.price)}
                {isSubject && <span style={{ fontSize: "clamp(8px, 0.9vw, 11px)", color: `${PARCH}99`, fontWeight: 400, marginLeft: 8 }}>{L.positionLead}</span>}
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
              <span style={{ background: isSubject ? RED : statusColor, color: "#fff", fontSize: "clamp(8px, 0.85vw, 10.5px)", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", padding: "4px 9px", borderRadius: 3 }}>
          ...
          {pics.length > 0 ? (
            ...
            <div style={{ padding: "26px 18px", textAlign: "center", color: `${PARCH}77`, fontSize: "clamp(9px, 1.05vw, 12.5px)", fontStyle: "italic" }}>
          ...
          {(isSubject ? subject.features : c.remarks) && (
            <div style={{ padding: "14px 18px", borderBottom: `1px solid ${BRASS}33` }}>
              <div style={{ fontSize: "clamp(8px, 0.9vw, 11px)", letterSpacing: "0.22em", textTransform: "uppercase", color: BRASS, fontWeight: 700, marginBottom: 10 }}>
                {isSubject ? "Features" : "Agent remarks"}
              </div>
              <div style={{ fontSize: "clamp(9.5px, 1.1vw, 13.5px)", lineHeight: 1.75, color: `${PARCH}DD`, whiteSpace: "pre-wrap", overflowWrap: "anywhere", wordBreak: "break-word" }}>
                {isSubject ? subject.features : c.remarks}
              </div>
            </div>
          )}
          ...
          {rows.length > 0 && (
            <div style={{ padding: "14px 18px 8px" }}>
              ...
              {rows.map(([k, v]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "7px 0", borderBottom: `1px solid ${PARCH}18`, fontSize: "clamp(9px, 1.05vw, 13px)" }}>
      ...
      case "cover":
        return (
          <div style={{ height: "100%", position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "5% 8%" }}>
            ...
            <div className="ds-head" style={{ fontSize: "clamp(14px, 1.9vw, 23px)", color: INK, marginTop: "1.5%" }}>
      ...
      case "intro":
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5% 7%" }}>
            ...
            <div style={{ flex: 1, display: "flex", gap: "5%", alignItems: "stretch", marginTop: "2.5%", minHeight: 0 }}>
              {agent.photo && (
              ...
              <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                <div className="ds-head" style={{ fontSize: "clamp(20px, 2.9vw, 38px)", fontWeight: 700, color: GREEN, letterSpacing: "-0.01em" }}>
                  {agent.name}
                </div>
                ...
                {ed("intro.body", agent.bio || "Add a short bio in Agent inputs — a couple of lines about your experience in this neighborhood.",
                  { fontSize: "clamp(11px, 1.5vw, 19px)", lineHeight: 1.75, color: INK, whiteSpace: "pre-wrap", overflowWrap: "anywhere" },
                  { block: true, className: "ds-head" })}
      ...
      case "details": {
        ...
        const col = (list) => (
          <div style={{ flex: 1, minWidth: 0 }}>
            {list.map(([k, v]) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "1.5% 0", borderBottom: `1px solid ${BRASS}22`, fontSize: "clamp(9px, 1.15vw, 14px)" }}>
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "4.5% 7%" }}>
            ...
            <div className="ds-head" style={{ fontSize: "clamp(16px, 2.2vw, 28px)", fontWeight: 700, color: INK, marginTop: "1.5%" }}>
              {subject.address}
            </div>
              ...
              <div style={{ flexShrink: 0, marginTop: "2%", paddingTop: "2%", borderTop: `1.5px solid ${BRASS}` }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5%", rowGap: 8 }}>
                  {feats.slice(0, 10).map((f, i) => (
                    <span key={i} style={{ background: CARD, color: INK, fontSize: "clamp(9px, 1.1vw, 13px)", padding: "5px 11px", borderRadius: 3, overflowWrap: "anywhere", maxWidth: "100%" }}>{f}</span>
                  ))}
                </div>
              </div>
            )}
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5% 8%" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", minHeight: 0 }}>
              <div className="ds-head" style={{ textAlign: "center", fontSize: "clamp(36px, 6vw, 82px)", fontWeight: 700, color: RED, lineHeight: 1, letterSpacing: "-0.02em" }}>
                {fmt(mid)}
              </div>
              ...
              <div style={{ width: "70%", margin: "6% auto 0" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: "3%" }}>
                  <div>
                    <div className="ds-head" style={{ fontSize: "clamp(16px, 2.15vw, 28px)", fontWeight: 600, color: INK }}>{fmt(lo)}</div>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div className="ds-head" style={{ fontSize: "clamp(16px, 2.15vw, 28px)", fontWeight: 600, color: INK }}>{fmt(hi)}</div>
      ...
      case "map": {
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "2.6% 3.6% 2.2%" }}>
            ...
            {/* Comp filmstrip */}
            <div style={{ display: "flex", gap: 6, overflowX: "auto", flexShrink: 0, margin: "1.1% 0 1%", paddingBottom: 4 }}>
              {strip.map((it, i) => (
                <button
                  key={i}
                  onClick={() => focusPin(i)}
                ...
                >
                  ...
                  <div style={{ padding: "4px 6px", fontSize: "clamp(7px, 0.78vw, 9.5px)", color: INK, fontWeight: 600, lineHeight: 1.3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {it.addr}
                  </div>
                </button>
              ))}
            ...
            <div style={{ flexShrink: 0, fontSize: "clamp(8px, 0.9vw, 11px)", color: MUTE, fontStyle: "italic", paddingTop: 7 }}>
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "4.5% 7%" }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: "1.5%", marginTop: "2%" }}>
              <div className="ds-head" style={{ fontSize: "clamp(15px, 2vw, 26px)", fontWeight: 700, color: BRASS }}>{slide.i + 1}</div>
              <div className="ds-head" style={{ fontSize: "clamp(19px, 2.7vw, 35px)", fontWeight: 700, color: INK, letterSpacing: "-0.01em" }}>{c.address}</div>
            </div>
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", minHeight: 0, overflow: "auto" }}>
              <div style={{ borderLeft: `4px solid ${BRASS}`, paddingLeft: "3.5%" }}>
                {ed("strategy.body", strategyDefault,
                  { fontSize: "clamp(14px, 1.95vw, 25px)", lineHeight: 1.7, color: INK, whiteSpace: "pre-wrap", overflowWrap: "anywhere" },
                  { block: true, className: "ds-head" })}
              </div>
            </div>
          </div>
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5.5% 8%" }}>
            ...
            <div style={{ flex: 1, minHeight: 0, overflow: "auto" }}>
              {box("body", c.body, { fontFamily: "'Fraunces', Georgia, serif", fontSize: "clamp(12px, 1.65vw, 21px)", lineHeight: 1.75, color: INK, whiteSpace: "pre-wrap", overflowWrap: "anywhere" })}
            </div>
          </div>
        );
      }
      ...
      case "close":
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "6% 9%" }}>
            ...
            <div className="ds-head" style={{ fontSize: "clamp(23px, 3.3vw, 44px)", fontWeight: 700, color: GREEN, letterSpacing: "-0.01em" }}>{agent.name}</div>
    ...
    <div style={{ position: "fixed", inset: 0, background: DARK, zIndex: 9999, display: "flex", flexDirection: "column", fontFamily: "'Libre Franklin', Arial, sans-serif" }}>
      <style>{`
        ...
        @media (max-width: 760px) and (orientation: portrait) {
          /* keep true slide proportions and centre them, like Google Slides */
          .ds-slidebox { aspect-ratio: 16 / 9 !important; width: 100% !important; height: auto !important; }
          .ds-rotate { display: flex !important; }
        }
        @media (max-width: 760px) {
          .ds-stage { padding: 6px 8px !important; }
          .ds-slidebox { width: 100% !important; }
          .ds-topbarP { padding: 8px 10px !important; }
      ...
      {/* Top bar */}
      <div className="ds-topbarP" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "12px 18px", borderBottom: "1px solid rgba(255,255,255,.06)", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button className="ds-btn ds-hide-sm" onClick={toggleFullscreen} title="Present full screen">
      ...
      <div className="ds-rotate" style={{ display: "none", alignItems: "center", justifyContent: "center", gap: 8, padding: "6px 14px 0", color: SAGE, fontSize: 11.5 }}>
        <span>Turn your phone sideways for a full-size presentation</span>
      </div>
