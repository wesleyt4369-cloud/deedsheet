...
export default function Presentation({
  ...
  const renderSlide = (slide) => {
    switch (slide.type) {
      case "cover":
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "4% 6%" }}>
            <div style={{ fontSize: "clamp(9px, 1vw, 12px)", letterSpacing: "0.35em", textTransform: "uppercase", color: RED, fontWeight: 700, fontFamily: "'Libre Franklin', Arial, sans-serif", marginBottom: "2%" }}>
              Prepared exclusively for the property owner
            </div>
            <h1 className="fraunces" style={{ fontSize: "clamp(26px, 4.2vw, 52px)", fontWeight: 700, color: GREEN, margin: 0 }}>
              Comparative Market Analysis
            </h1>
            <div className="fraunces" style={{ fontSize: "clamp(15px, 1.9vw, 24px)", color: INK, marginTop: "1.5%" }}>
              {subject.address} · {subject.city}
            </div>
            <div style={{ fontSize: "clamp(11px, 1.2vw, 14px)", color: MUTE, marginTop: "1%" }}>
              {subject.beds} bed · {subject.baths} bath · {(subject.sqft || 0).toLocaleString()} sqft · {subject.lot} lot · built {subject.year}
            </div>
            {subject.photo && (
              <img src={subject.photo} alt={subject.address} style={{ marginTop: "2.5%", width: "52%", maxHeight: "38%", objectFit: "cover", border: `1.5px solid ${BRASS}` }} />
            )}
            <div style={{ marginTop: "auto", fontSize: "clamp(10px, 1.1vw, 13px)", color: MUTE }}>
              Prepared by {agent.name} · {agent.brokerage} · {new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
            </div>
          </div>
        );
      case "position":
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5% 7%" }}>
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", gap: "4%" }}>
              {[
                { label: "LOW", val: fmt(lo), color: INK, size: "clamp(20px, 3vw, 38px)" },
                { label: "RECOMMENDED LIST", val: fmt(mid), color: RED, size: "clamp(30px, 4.8vw, 62px)" },
                { label: "HIGH", val: fmt(hi), color: INK, size: "clamp(20px, 3vw, 38px)" },
              ].map((c) => (
                <div key={c.label} style={{ textAlign: "center", flex: 1 }}>
                  <div className="fraunces" style={{ fontSize: c.size, fontWeight: 700, color: c.color }}>{c.val}</div>
                  <div style={{ fontSize: "clamp(9px, 1vw, 12px)", letterSpacing: "0.15em", color: MUTE, marginTop: 8 }}>{c.label}</div>
                </div>
              ))}
            </div>
            <div style={{ textAlign: "center", fontSize: "clamp(11px, 1.2vw, 14px)", color: MUTE, fontStyle: "italic" }}>
      ...
      case "map":
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "4% 5%" }}>
            {eyebrow("Comparable locations")}
            <div ref={mapDivRef} style={{ flex: 1, marginTop: "1.5%", border: `1.5px solid ${BRASS}` }} />
            <div style={{ fontSize: "clamp(10px, 1.05vw, 12px)", color: MUTE, fontStyle: "italic", marginTop: 8 }}>
