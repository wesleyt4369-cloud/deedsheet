...
export default function Presentation({
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: DARK, zIndex: 9999, display: "flex", flexDirection: "column", fontFamily: "'Libre Franklin', Arial, sans-serif" }}>
      ...
      >
        {isSmall ? (
          <div style={{ width: Math.max(160, Math.round(CANVAS_W * (scale || 0.25))), height: Math.max(90, Math.round(CANVAS_H * (scale || 0.25))), position: "relative", flexShrink: 0 }}>
            <div className="ds-slide ds-slidebox" key={slides[idx]?.key || idx}
              style={{
                containerType: "inline-size", width: CANVAS_W, height: CANVAS_H,
                transform: `scale(${scale || 0.25})`, transformOrigin: "top left",
        ...
        <div className="ds-slide ds-slidebox" key={slides[idx]?.key || idx}
          style={{ containerType: "inline-size", aspectRatio: "16 / 9", width: "min(100%, 168vh)", maxWidth: "100%", maxHeight: "100%", background: PARCH, boxShadow: "0 20px 60px rgba(0,0,0,.5)", position: "relative", opacity: (d.hidden || {})[slides[idx]?.key] ? 0.55 : 1 }}>
