...
export default function Presentation({
  ...
  const slides = useMemo(() => {
    const bs = [{ type: "cover", key: "cover" }, { type: "position", key: "position" }];
  ...
  }, [adjusted, mapPoints, geoKey, ns.enabled, d, editing]);
  ...
  const renderSlide = (slide) => {
    ...
    switch (slide.type) {
      case "cover":
        return (
          <div style={{ height: "100%", position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "4% 8%" }}>
            ...
            {ed("cover.title", "Comparative Market Analysis",
              { fontSize: "clamp(26px, 4.3vw, 54px)", fontWeight: 700, color: GREEN, marginTop: "1.8%", lineHeight: 1.08, letterSpacing: "-0.01em" },
              { block: true, className: "fraunces" })}
            <div className="fraunces" style={{ fontSize: "clamp(14px, 1.95vw, 24px)", color: INK, marginTop: "1.6%" }}>
              {subject.address} · {subject.city}
            </div>
            <div style={{ fontSize: "clamp(10px, 1.15vw, 13px)", color: MUTE, marginTop: "0.9%", letterSpacing: "0.02em" }}>
              {subject.beds} bed · {subject.baths} bath · {(subject.sqft || 0).toLocaleString()} sqft · {subject.lot} lot · built {subject.year}
            </div>
            {subject.photo ? (
              <img src={subject.photo} alt={subject.address} style={{ marginTop: "3%", width: "48%", maxHeight: "33%", objectFit: "cover", border: `1.5px solid ${BRASS}`, boxShadow: "0 6px 18px rgba(0,0,0,0.14)" }} />
            ) : (
              <div style={{ marginTop: "3.5%", width: "34%", height: 1, background: BRASS, opacity: 0.45 }} />
            )}
            <div style={{ marginTop: "auto", paddingTop: "3%" }}>
              <div style={{ width: 46, height: 1.5, background: BRASS, margin: "0 auto 3%" }} />
