...
export default function Presentation({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const addChip = (lnglat, bg, fg, label, sub, popupHtml, z) => {
        const m = L.marker([lnglat[1], lnglat[0]], {
          icon: L.divIcon({ className: "", html: chipHtml(bg, fg, label, sub), iconSize: [1, 1], iconAnchor: [0, 26], popupAnchor: [0, -30] }),
          zIndexOffset: z,
      ...
      const markers = [];
      markers.push(addChip(
        mapPoints.subj, RED, "#FFFFFF", "SUBJECT", "S",
        `<div style="font-family:Georgia,serif;min-width:190px;"><b style="font-size:14px;">${subject.address}</b><br/><span style="color:#555;">${subject.beds} bd / ${subject.baths} ba · ${(subject.sqft || 0).toLocaleString()} sqft</span><br/><span style="color:#8e3b2f;font-weight:700;">${L.positionLead}: ${fmt(mid)}</span></div>`,
        1000
      ));
      (mapPoints.comps || []).forEach((coord, i) => {
        ...
        const c = adjusted[i];
        markers.push(addChip(
          coord, "#2B2B2B", "#FFFFFF", shortPrice(c.price), String(i + 1),
          `<div style="font-family:Georgia,serif;min-width:190px;"><b style="font-size:14px;">${i + 1}. ${c.address}</b><br/><span style="color:#555;">${c.beds} bd / ${c.baths} ba · ${(c.sqft || 0).toLocaleString()} sqft · sold ${c.sold}</span><br/>Sold: <b>${fmt(c.price)}</b><br/>Adjusted: <b>${fmt(c.adjValue)}</b></div>`,
          0
        ));
      });
      ...
      case "map": {
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "3.2% 4.5% 2.6%" }}>
            ...
            <div ref={mapDivRef} style={{ flex: 1, minHeight: 0, border: `1.5px solid ${BRASS}`, boxShadow: "0 4px 14px rgba(0,0,0,0.1)" }} />
            <div style={{ flexShrink: 0, fontSize: "clamp(8px, 0.9vw, 11px)", color: MUTE, fontStyle: "italic", paddingTop: 7 }}>
              Click a photo above to fly to that property · price chips show the sold price · drag and zoom the map · switch to Overhead for satellite view
            </div>
          </div>
        );
      }
