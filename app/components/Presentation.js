...
export default function Presentation({
  ...
  const addCustomSlide = (afterKey) => {
    ...
    onDeckChange({
      ...d,
      custom: [...(d.custom || []), { id, after: afterKey, title: "New slide", body: "Click any text to edit it." }],
    });
  };
  const updateCustom = (id, field, value) => {
    ...
    onDeckChange({
      ...d,
      custom: (d.custom || []).map((c) => (c.id === id ? { ...c, [field]: value } : c)),
    });
  };
  ...
  const ed = (tkey, defText, style = {}, opts = {}) => {
    ...
    if (!editing) {
      return <Tag className={opts.className} style={style}>{value}</Tag>;
    }
    return (
      <Tag
        className={opts.className}
        style={{ ...style, outline: `1px dashed ${BRASS}`, outlineOffset: 3, cursor: "text", minWidth: 30 }}
        contentEditable
  ...
  const slides = useMemo(() => {
    const base = [{ type: "cover", key: "cover" }, { type: "position", key: "position" }];
    if (mapPoints && geoKey) base.push({ type: "map", key: "map" });
    adjusted.forEach((_, i) => base.push({ type: "comp", i, key: `comp${i}` }));
    base.push({ type: "strategy", key: "strategy" });
    if (netSheet?.enabled) base.push({ type: "net", key: "net" });
    base.push({ type: "close", key: "close" });

    // Weave in custom slides after whichever slide they were added to
    const customs = d.custom || [];
    const out = [];
    base.forEach((s) => {
      out.push(s);
      customs
        .filter((c) => c.after === s.key)
        .forEach((c) => out.push({ type: "custom", id: c.id, key: `custom:${c.id}` }));
    });
    ...
    // Hidden slides stay visible while editing so they can be restored
    return editing ? out : out.filter((s) => !(d.hidden || {})[s.key]);
  }, [adjusted, mapPoints, geoKey, netSheet, d, editing]);
  ...
  const eyebrowStyle = { fontSize: "clamp(9px, 1.1vw, 13px)", letterSpacing: "0.3em", textTransform: "uppercase", color: GREEN, fontWeight: 700, fontFamily: "'Libre Franklin', Arial, sans-serif" };
  const eyebrow = (text, tkey) => ed(tkey, text, eyebrowStyle, { block: true });
  ...
  const renderSlide = (slide) => {
    switch (slide.type) {
        ...
        return (
          <div style={{ height: "100%", position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "3% 7%" }}>
            ...
            {ed("cover.eyebrow", "Prepared exclusively for the property owner",
              { fontSize: "clamp(9px, 1vw, 12px)", letterSpacing: "0.35em", textTransform: "uppercase", color: RED, fontWeight: 700, fontFamily: "'Libre Franklin', Arial, sans-serif" },
              { block: true })}
            {ed("cover.title", "Comparative Market Analysis",
              { fontSize: "clamp(26px, 4.4vw, 54px)", fontWeight: 700, color: GREEN, margin: "1.5% 0 0", display: "block" },
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center" }}>
              <div style={{ borderLeft: `4px solid ${BRASS}`, paddingLeft: "3%" }}>
                {ed("strategy.body", strategyDefault,
                  { fontSize: "clamp(15px, 2vw, 25px)", lineHeight: 1.65, color: INK, margin: 0 },
                  { block: true, className: "fraunces" })}
              </div>
            </div>
          </div>
      ...
      case "net":
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "4% 10%" }}>
            {eyebrow("Estimated seller net proceeds", "net.eyebrow")}
            <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", minHeight: 0 }}>
              <div style={{ background: CARD, padding: "3% 4.5%" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "clamp(13px, 1.6vw, 19px)", fontWeight: 700, color: INK, padding: "6px 0" }}>
                  <span>Sale price</span><span>{fmt(netBase)}</span>
                </div>
                {netLines.map(([label, v]) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", fontSize: "clamp(11px, 1.3vw, 16px)", color: MUTE, padding: "5px 0 0 4%" }}>
                    <span>{label}</span>
                    <span style={{ color: RED, fontWeight: 600 }}>−{fmt(v)}</span>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "clamp(16px, 2.2vw, 27px)", fontWeight: 700, color: GREEN, paddingTop: 12, marginTop: 12, borderTop: `1.5px solid ${BRASS}` }}>
                  <span>Estimated net to seller</span><span>{fmt(netProceeds)}</span>
                </div>
              </div>
              <div style={{ fontSize: "clamp(9px, 0.95vw, 11px)", color: "#8A7E63", fontStyle: "italic", marginTop: "2%", textAlign: "center" }}>
      ...
      case "custom": {
        ...
        if (!c) return null;
        const editBox = (field, value, style) =>
          editing ? (
            <div
              style={{ ...style, outline: `1px dashed ${BRASS}`, outlineOffset: 4, cursor: "text" }}
              contentEditable
              suppressContentEditableWarning
        ...
        return (
          <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "5% 8%" }}>
            {editBox("title", c.title, { fontFamily: "'Fraunces', Georgia, serif", fontSize: "clamp(22px, 3.2vw, 42px)", fontWeight: 700, color: GREEN })}
            <div style={{ width: "18%", height: 1.5, background: BRASS, margin: "2.5% 0" }} />
            <div style={{ flex: 1, minHeight: 0, overflow: "auto" }}>
              {editBox("body", c.body, { fontFamily: "'Fraunces', Georgia, serif", fontSize: "clamp(13px, 1.7vw, 21px)", lineHeight: 1.7, color: INK, whiteSpace: "pre-wrap" })}
            </div>
          </div>
        );
      }
    ...
    <div style={{ position: "fixed", inset: 0, background: DARK, zIndex: 9999, display: "flex", flexDirection: "column", fontFamily: "'Libre Franklin', Arial, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "10px 16px", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          {editable && barBtn(editing ? "Done editing" : "Edit slides", () => setEditing(!editing), { solid: editing })}
          {onShare && barBtn(
      ...
      {editing && (
        <div style={{ borderTop: "1px solid #2E5540", padding: "10px 14px", display: "flex", gap: 8, alignItems: "center", overflowX: "auto" }}>
          {slides.map((s, i) => {
            ...
            const label =
              ...
              : ((d.custom || []).find((c) => c.id === s.id)?.title || "Custom").slice(0, 16);
            return (
              <div key={s.key}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 8px", borderRadius: 3, flexShrink: 0,
                  border: `1px solid ${i === idx ? BRASS : "#2E5540"}`, background: i === idx ? "rgba(168,133,60,0.15)" : "transparent",
                  opacity: isHidden ? 0.45 : 1 }}>
                <button onClick={() => setIdx(i)}
                  style={{ background: "none", border: "none", color: PARCH, fontSize: 12, cursor: "pointer", padding: 0, textDecoration: isHidden ? "line-through" : "none" }}>
                  {i + 1}. {label}
                </button>
                <button onClick={() => toggleHidden(s.key)} title={isHidden ? "Show slide" : "Hide slide"}
                  style={{ background: "none", border: "none", color: "#8FAE9B", fontSize: 12, cursor: "pointer", padding: 0 }}>
                  {isHidden ? "show" : "hide"}
                </button>
                {s.type === "custom" && (
                  <button onClick={() => deleteCustom(s.id)} title="Delete slide"
                    style={{ background: "none", border: "none", color: "#E08A7A", fontSize: 12, cursor: "pointer", padding: 0 }}>
                    ✕
                  </button>
                )}
              </div>
          ...
          <button onClick={() => addCustomSlide(slides[idx]?.key)}
            style={{ flexShrink: 0, padding: "7px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", borderRadius: 3, border: `1px dashed ${BRASS}`, background: "transparent", color: "#D9C48F" }}>
            + Add slide here
          </button>
        </div>
      )}
      <div style={{ textAlign: "center", padding: "6px 0 12px", color: "#8FAE9B", fontSize: 12 }}>
        {idx + 1} / {slides.length} · use ← → keys{editing ? " · click any text on the slide to edit it" : ""}
      </div>
    </div>
  );
}
