...
// Resizes an uploaded image. Transparent formats stay PNG so logos keep their
// transparency; photographs become JPEG so drafts stay light.
const fileToResizedDataUrl = (file) =>
  new Promise((resolve, reject) => {
    const type = (file && file.type) || "";
    const keepsAlpha = /png|webp|gif|svg/i.test(type);
    const img = new Image();
    const reader = new FileReader();
    reader.onload = () => { img.src = reader.result; };
    reader.onerror = reject;
    img.onload = () => {
      const max = keepsAlpha ? 900 : 1200;
      const scale = Math.min(1, max / Math.max(img.width, img.height));
      const canvas = document.createElement("canvas");
      canvas.width = Math.max(1, Math.round(img.width * scale));
      canvas.height = Math.max(1, Math.round(img.height * scale));
      const ctx = canvas.getContext("2d");
      // no background fill — an untouched canvas is transparent
      ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
      resolve(keepsAlpha ? canvas.toDataURL("image/png") : canvas.toDataURL("image/jpeg", 0.82));
    };
    img.onerror = reject;
...
export default function DeedSheet() {
  ...
  // Every image upload goes through the cropper first
  const requestCrop = (file, { aspect, label }, apply) => {
    if (!file) return;
    ...
    reader.onload = () => {
      setCropReq({
        ...
        label,
        keepAlpha,
        apply: (dataUrl) => { apply(dataUrl); setCropReq(null); },
      });
    };
    reader.readAsDataURL(file);
  };
  ...
  const setProfileImage = (field, file) =>
    requestCrop(
      file,
      field === "logo"
        ? { aspect: 3 / 1, label: "Position your logo" }
        : { aspect: 4 / 5, label: "Position your headshot" },
      (dataUrl) => setProfileDraft((prev) => ({ ...prev, [field]: dataUrl }))
    );
  ...
  const setSubjectPhoto = (file) =>
    requestCrop(file, { aspect: 16 / 9, label: "Position the cover photo" },
      (dataUrl) => setSubject((prev) => ({ ...prev, photo: dataUrl })));

  const setAgentImage = (field, file) =>
    requestCrop(
      file,
      field === "logo"
        ? { aspect: 3 / 1, label: "Position your logo" }
        : { aspect: 4 / 5, label: "Position your headshot" },
      (dataUrl) => setAgent((prev) => ({ ...prev, [field]: dataUrl }))
    );

  const addCompPhotos = (compId, fileList) => {
    const files = Array.from(fileList || []);
    if (!files.length) return;
    // Walk the queue so each photo gets positioned before the next one opens
    const next = (i) => {
      if (i >= files.length) return;
      requestCrop(
        files[i],
        { aspect: 4 / 3, label: `Position photo ${i + 1} of ${files.length}` },
        (dataUrl) => {
          setComps((prev) =>
            prev.map((c) =>
              c.id === compId
                ? { ...c, photos: [...(c.photos || []), dataUrl], photo: c.photo || dataUrl }
                : c
            )
          );
  ...
  const setCompPhoto = (compId, file) =>
    requestCrop(file, { aspect: 4 / 3, label: "Position the comp photo" },
      (dataUrl) => setComps((prev) => prev.map((c) => (c.id === compId ? { ...c, photo: dataUrl } : c))));
  ...
  const fonts = `
    ...
    @keyframes dsSlide { 0% { transform: translateX(-100%); } 100% { transform: translateX(320%); } }
  ...
  // Don't show the app for a split second before the paywall — wait for the answer
  if (sub.loading) {
    return (
      <div style={{ background: "#1F3D2B", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Libre Franklin', Arial, sans-serif" }}>
        <style>{fonts}</style>
        <div style={{ textAlign: "center" }}>
          <div className="fraunces" style={{ fontSize: 24, fontWeight: 700, color: "#EFE7D3" }}>DeedSheet</div>
          <div style={{ fontSize: 12.5, color: "#8FAE9B", marginTop: 8, letterSpacing: "0.08em" }}>Checking your subscription…</div>
          <div style={{ width: 120, height: 3, background: "rgba(255,255,255,.08)", borderRadius: 2, margin: "16px auto 0", overflow: "hidden" }}>
            <div style={{ width: "40%", height: "100%", background: "#A8853C", borderRadius: 2, animation: "dsSlide 1.1s ease-in-out infinite" }} />
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ background: "#1F3D2B", minHeight: "100vh", fontFamily: "'Libre Franklin', sans-serif" }}>
      ...
      {cropReq && (
        <ImageCropper
          ...
          label={cropReq.label}
          keepAlpha={cropReq.keepAlpha}
          onCancel={() => setCropReq(null)}
          onApply={cropReq.apply}
        />
      )}
